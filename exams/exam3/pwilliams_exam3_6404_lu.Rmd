...
```{r thresh_comps, include = T, echo = F}
#thresholding https://cran.r-project.org/web/packages/waveslim/waveslim.pdf
wv <- dwt(x, wf="haar", n.levels=4, boundary="periodic")
# da.thresh(wv, alpha = .1, max.level = 4, verbose = T, return.thresh = T)
# hybrid.thresh(wv, max.level = 4, verbose = FALSE, seed = 0)
# manual.thresh(wv, max.level = 4, 0.1, hard = TRUE)
# sure.thresh(wv, max.level = 4, hard = TRUE)
# universal.thresh(wv, max.level = 4, hard = TRUE)
# universal.thresh.modwt(wv, max.level = 4, hard = TRUE)
```
...
The scatter plot again shows that the spline provides a slightly more conservative predictin vs. the kernal regression. However, both predictions do lie in a reasonable place relative to actual data points observed. 
...
#### Multi-resolution Plot Generation (waveslim)
...
B <- 1000 #bootstrap replication count
#estimator function
